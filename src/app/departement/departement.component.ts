...
import { Users, UsersService } from './departement.service';
...
export class DepartementComponent implements OnInit{
  ...
  fetchUsers(): void {
    this.usersService.getUsers().subscribe(
      (data: Users[]) => {
        this.departements = data; // Set the data source with the fetched users
      },
      (error) => {
  ...
  deleteDepartement(departementId: string): void {
    if (confirm('Are you sure you want to delete this user?')) {
    this.usersService.deleteUser(departementId).subscribe(
      () => {
